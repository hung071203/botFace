...
module.exports.run = async function (api, event, args, client) {
    ...
    switch (args[1]) {
        ...
        case 'buy':
            ...
            switch (args[2]) {
                case 'coin':
                    if(player.money < number) return api.sendMessage('Bạn không đủ tiền!', event.threadID, event.messageID)
                    player.money -= number
                    find.coin += number
                ...
                case 'spin':
                    if(player.money < number * 100) return api.sendMessage('Bạn không đủ tiền!', event.threadID, event.messageID)
