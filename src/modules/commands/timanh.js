...
module.exports.run = async function (api, event, args, client) {
    ...
    pingURL(`https://www.google.gg/search?q=${encodeURIComponent(query)}&sca_esv=594000818&tbm=isch&source=lnms&sa=X&ved=2ahUKEwid3_LR9a-DAxWzbPUHHRNjBKcQ_AUoAXoECAIQAw&biw=1912&bih=924&dpr=1`);
    ...
    try {
      await page.waitForSelector('img[id*="dimg_"]', { timeout: 5000 });
    ...
    const imageSrcList = await page.evaluate(() => {
      const imageElements = document.querySelectorAll('img[id*="dimg_"]');
      const srcList = Array.from(imageElements).slice(0, 6).map(img => img.src.replace(/^data:image\/(jpeg|png);base64,/, ''));
      return srcList;
    });
    await browser.close();
    let arrimg =[];
